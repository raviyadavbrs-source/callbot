    ...
    try:
        while True:
            ...
            if event == 'start':
                ...
                # Start Deepgram
                lang = 'hi' if session.get('bot') == 'bunty' else 'en-IN'  # Bunty=Hindi, Jessica=English Indian
...
var activeCallSid = null, activeCallPrefix = null, pollInt = null;

function startCallPoll(prefix, bot) {
  if(pollInt) clearInterval(pollInt);
  pollInt = setInterval(function(){
    fetch('/jessica/logs').then(function(r){return r.json();}).then(function(logs){
      if(!logs || !logs.length) return;
      var latest = logs[0];
      if(latest && ['completed','failed','busy','no-answer','canceled'].indexOf(latest.status) >= 0) {
        clearInterval(pollInt); pollInt = null;
        stopTimer(prefix);
        var btn = document.getElementById(prefix+'-btn');
        btn.disabled = false;
        btn.textContent = bot==='bunty' ? '\u260e Call Maar' : '\u260e Call Now';
        showSt(prefix+'-status', 'Call ended: ' + latest.status, latest.status==='completed'?'success':'error');
        loadLogs();
      }
    }).catch(function(){});
  }, 4000);
...
async function makeCall(bot, callType, phone, name, extra) {
  ...
  try{
    ...
    if(res.ok){ showSt(prefix+'-status','Calling '+name+'...\u2705','success'); startTimer(prefix); btn.textContent='On Call...'; startCallPoll(prefix, bot); }
...
function loadLogs(){
  fetch('/jessica/logs').then(function(r){return r.json();}).then(function(logs){
    ...
    for(var i=0;i<logs.length;i++){
      ...
      var tr='';
      if(log.transcript&&log.transcript.length){
        tr='<div class="transcript" id="tr-'+log.call_sid+'" style="display:none">';
        for(var j=0;j<log.transcript.length;j++){
          var parts=log.transcript[j].split(': ');
          tr+='<div class="tr-line"><span class="tr-who">'+parts[0]+':</span> '+parts.slice(1).join(': ')+'</div>';
        }
        tr+='</div>';
      }
      html+='<div class="log-card">';
      html+='<div class="log-top"><div>';
      html+='<div class="log-name">'+log.friend_name+' <span style="font-size:11px;color:#666;">'+bl+'</span></div>';
      html+='<div class="log-meta">'+badge+' &bull; '+log.timestamp+' &bull; '+log.duration+'s</div>';
      html+='</div><div class="log-badge '+(log.status==='completed'?'ok':'na')+'">'+log.status+'</div></div>';
      html+='<button class="tr-btn" onclick="toggleTr('+JSON.stringify(log.call_sid)+')">&#128172; Transcript</button>';
      html+=tr+'</div>';
    }
    c.innerHTML=html;
  }).catch(function(){});
